...
;define("landscape/controllers/landscapes", ["exports"], function (_exports) {
  "use strict";
  ...
  class Landscapes extends Ember.Controller {// @service('landscape-token')
...
;define("landscape/routes/landscapes", ["exports", "landscape/config/environment"], function (_exports, _environment) {
  ...
  let Landscapes = (_dec = Ember.inject.service('landscape-token'), _dec2 = Ember._action, _dec3 = Ember._action, (_class = class Landscapes extends Ember.Route {
    ...
    refreshRoute() {// return this.refresh();
    }
    ...
    async model() {// let uId = this.auth.user?.sub;
      // let uId = JSON.parse(sessionStorage.getItem('user') as string)?.sub;
      // if (!uId) {
      //   return Promise.reject(new Error('User profile not set'));
      // }
    ...
    afterModel(landscapeTokens) {// const currentToken = this.tokenService.token;
